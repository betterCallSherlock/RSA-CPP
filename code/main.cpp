...
void generateRSAKeys(long long p, long long q, long long &n, long long &e, long long &d) {
    n = p * q;
    long long phi = EulerFunction(p, q);
...
int main() {
    long long n,e,d;
    generateRSAKeys(generateLargePrime(10,10),generateLargePrime(10,10),
        n,e,d);

    long long message = 2022;
    long long cipher = encrypt(message,e,n);
    long long decrypted_cipher = decrypt(cipher,d,n);
    std::cout<<"message: "<<message<<" cipher:"<<cipher<<
    " decrypted_cipher:"<<decrypted_cipher<<std::endl;
    return 0;
}
